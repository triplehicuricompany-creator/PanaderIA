      ...
      <div className="grid gap-5 md:grid-cols-2">
        {digitalLibrary.map((item) => (
          <article key={item.title} className="rounded-3xl border border-gold/20 bg-charcoal p-7">
            ...
            <p className="mt-3 text-cream/65">Formato {item.format}. Recurso existente vinculado al módulo correspondiente; en producción podrá protegerse con URLs firmadas.</p>
            <a href={item.href} className="mt-6 inline-flex rounded-full border border-gold/50 px-5 py-3 font-bold text-gold">Abrir recurso</a>
          </article>
        ))}
      </div>
    </StudentShell>
