    ...
    <section className="overflow-x-hidden px-4 py-6 sm:px-5 sm:py-10">
      <div className="mx-auto grid w-full max-w-7xl gap-5 lg:grid-cols-[240px_1fr] lg:gap-8">
        <aside className="rounded-[1.75rem] border border-toast/10 bg-white/70 p-3 shadow-soft backdrop-blur lg:sticky lg:top-28 lg:h-fit lg:p-5">
          ...
          <nav className="mt-3 grid grid-cols-2 gap-2 pb-1 sm:grid-cols-3 lg:grid-cols-1" aria-label="Navegación del alumno">
            {studentNav.map((item) => (
              <Link key={item.href} href={item.href} className="flex min-h-11 items-center rounded-2xl px-4 py-2 text-sm font-bold text-cocoa/70 transition hover:bg-peach/45 hover:text-toast focus:outline-none focus:ring-4 focus:ring-coral/15">
                {item.label}
              </Link>
            ))}
          </nav>
